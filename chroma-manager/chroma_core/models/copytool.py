...
class CopytoolOperation(models.Model):
    ...
    def __str__(self):
        return "%s %s" % (self.STATE_CHOICES[self.state][1],
                          self.TYPE_CHOICES[self.type][1] % self.path)
    ...
    class Meta:
        ...
        unique_together = ('state', 'copytool', 'fid',
...
class Copytool(StatefulObject, MeasuredEntity):
    ...
    def get_label(self):
        return "%s-%s-%s-%s" % (os.path.basename(self.bin_path),
                                self.filesystem.name, self.archive, self.index)
    ...
    def create_operation(self, start_time, type, path, fid):
        try:
            return self.operations.create(started_at = start_time,
                                          state = OP_STATES.STARTED,
                                          type = resolve_value('type', type),
                                          path = path, fid = fid)
        except KeyError:
    ...
    def current_operations(self):
        return self.operations.exclude(state__in=[OP_STATES.FINISHED,
                                                  OP_STATES.ERRORED])
    ...
    def get_deps(self, state = None):
        ...
        client_mount = ObjectCache.get_one(LustreClientMount,
                                           lambda cm: cm.id == self.client_mount_id)

        deps = []
        if state == 'started':
            # Depend on the client mount being mounted in order to
            # start or stay running.
            deps.append(DependOn(client_mount, 'mounted',
                                 fix_state='stopped'))
    ...
    reverse_deps = {
        'LustreClientMount': lambda cm: ObjectCache.client_mount_copytools(cm.id)
    }
...
class StartCopytoolJob(StateChangeJob):
    ...
    def get_steps(self):
        return [(StartCopytoolStep, {'host': self.copytool.host,
...
class StopCopytoolJob(StateChangeJob):
    ...
    def get_steps(self):
        return [(CancelActiveOperationsStep, {'copytool': self.copytool}),
                (StopCopytoolStep, {'host': self.copytool.host,
                                    'copytool': self.copytool})]
...
class UnconfigureCopytoolStep(Step):
    def run(self, kwargs):
        ...
        self.invoke_agent(host, "unconfigure_copytool",
                          {'id': str(copytool.id)})
